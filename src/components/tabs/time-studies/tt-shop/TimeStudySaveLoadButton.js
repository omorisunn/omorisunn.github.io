...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      const presetName = this.name ? `时间研究预设 "${this.name}" ` : "时间研究预设";
      GameUI.notify.eternity(`${presetName}已保存到槽位 ${this.saveslot}`);
    },
    load() {
      this.hideContextMenu();
      if (this.preset.studies) {
        ...
        const presetName = this.name ? `时间研究预设 "${this.name}" ` : "时间研究预设";
        GameUI.notify.eternity(`${presetName}已从槽位 ${this.saveslot} 加载`);
      } else {
        Modal.message.show("这个时间研究预设目前不包含时间研究。");
      }
    },
    respecAndLoad() {
    ...
    deletePreset() {
      ...
      else Modal.message.show("这个时间研究预设目前不包含时间研究。");
    },
    handleExport() {
      this.hideContextMenu();
      copyToClipboard(this.preset.studies);
      const presetName = this.name ? `时间研究预设 "${this.name}" ` : "时间研究预设";
      GameUI.notify.eternity(`${presetName}已从槽位 ${this.saveslot} 导出到剪贴板`);
    },
    edit() {
  ...
  >
    ...
    <template #menu>
      <div
        ...
        data-v-time-study-save-load-button
      >
        <span ach-tooltip="自定义名称 (至多四位英文、数字或符号)">
          <input
            type="text"
            size="4"
            maxlength="4"
