...
export function gameLoop(passDiff, options = {}) {
  ...
  if (diff === undefined) {
    diff = Enslaved.nextTickDiff * (player.options.externalSpeed ?? 1);
  }
      ...
      ticks,
      {
        ...
        asyncEntry: doneSoFar => {
          ...
          ui.$viewModel.modal.progressBar = {
            label: "离线进度模拟",
            info: () => `游戏正在以较低的精度运行，以便快速计算你离开期间获得的资源。
              有关技术细节，请参阅「如何游玩」中的「离线进度」条目。如果你等不及想要
              尽快回到游戏中，可以点击「加速」按钮，以一半的模拟次数（最少 ${formatInt(500)} 次）
              模拟剩余的时间。「跳过」按钮则会用 ${formatInt(10)} 次模拟全部剩余离线时间。`,
            progressName: "模拟次数",
            current: doneSoFar,
            max: ticks,
            startTime: Date.now(),
...
export function browserCheck() {
  // 原版用正则白名单匹配 UA,容易误伤新版 Safari/WebView。
  // 放宽:排除老 IE(documentMode),其余现代浏览器一律放行。
  if (window.documentMode !== undefined) return false;
  return true;
}
